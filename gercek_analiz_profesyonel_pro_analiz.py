...
def poisson_mac_tahmini(home_lambda, away_lambda):
    """Genişletilmiş skor dağılımı.

    Bağımsız Poisson ana modeldir. 0-15 gol aralığı kullanılır ve düşük
    skorların olasılıkları ayrıca normalize edilir. Bu fonksiyon olasılıkları
    yapay olarak yükseltmez; daha fazla skor senaryosunu hesaba katar.
    """
    ...
    ev_norm = sum(ev_gol.values()) or 1.0
    dep_norm = sum(dep_gol.values()) or 1.0

    ev_gol = {k: v / ev_norm for k, v in ev_gol.items()}
    dep_gol = {k: v / dep_norm for k, v in dep_gol.items()}
...
def gelismis_korner_model(home_corner, away_corner):
    """Korner modeli: beklenen değer + ampirik çizgi + over-dispersion.

    Korner sayıları futbolda gollere göre daha değişken olduğundan tek Poisson
    yerine düşük/orta/yüksek tempo senaryolarının karışımı kullanılır. Çizgiler
    ayrıca monotonicity kuralıyla düzeltilir: 7.5 üst >= 8.5 üst >= ...
    """
    if not isinstance(home_corner, dict) or not isinstance(away_corner, dict):
        return None
    ...
    hf = float(home_corner.get("korner_for", 0) or 0)
    ha = float(home_corner.get("korner_against", 0) or 0)
    af = float(away_corner.get("korner_for", 0) or 0)
    aa = float(away_corner.get("korner_against", 0) or 0)

    # Takımın ürettiği korner + rakibin verdiği korner.
    home_expected = hf * 0.58 + aa * 0.42
    away_expected = af * 0.58 + ha * 0.42
    model_total = home_expected + away_expected
    ...
    # Küçük örneklemde geçmiş ortalamaya biraz daha az, doğrudan eşleşme
    # modeline biraz daha fazla ağırlık verilir.
    n = hn + an
    observed_weight = min(0.30, 0.10 + n / 120.0)
    predicted_total = model_total * (1 - observed_weight) + observed_total * observed_weight
    predicted_total = max(4.5, min(16.0, predicted_total))

    thresholds = {
        "over75": 8, "over85": 9, "over95": 10,
        "over105": 11, "over115": 12, "over125": 13,
    }
    result = {
        "mac": n,
    ...
    for key, minimum in thresholds.items():
        ...
        empirical = []
        for rec in (home_corner, away_corner):
            if key in rec:
                try:
                    empirical.append(float(rec[key]))
                except Exception:
                    pass
        empirical_pct = sum(empirical) / len(empirical) if empirical else model_pct
        # 0-100 örnek sayısını gerçek veri lehine artırır; ancak model omurgası
        # tamamen kaybolmaz.
        pct = bayes_oran(empirical_pct, model_pct, n, prior_strength=16)
        result["lines"][key] = max(1.0, min(99.0, pct))

    # Matematiksel tutarlılık: daha yüksek üst çizgisinin olasılığı daha yüksek
    # olamaz. Böylece 9.5 ve 10.5 gibi çizgiler gerçekten ayrışır.
    keys = ["over75", "over85", "over95", "over105", "over115", "over125"]
    for i in range(1, len(keys)):
        result["lines"][keys[i]] = min(result["lines"][keys[i]], result["lines"][keys[i-1]] - 0.1)
    return result
...
def _odds_probability(odds_parsed, bet_names, value_names=None):
    """API-Football oranlarından marjı normalize edilmiş olasılık üretir.

    Birden fazla bookmaker varsa bookmaker ortalaması alınır. Oran verisi
    yoksa None döner; model oranı zorla kullanmaz.
    """
    if not odds_parsed:
        return None
    value_names = value_names or []
    candidates = []
    wanted = {str(x).strip().lower() for x in value_names}

    for bet_name, values in odds_parsed.items():
        if not any(str(bet_name).strip().lower() == str(b).strip().lower() for b in bet_names):
            continue
        for item in values or []:
            ...
            try:
                ...
                continue
            if odd > 1.01 and odd < 100:
                candidates.append(1.0 / odd)

    if not candidates:
        return None
    # Aynı market için farklı bookmaker kayıtları olabilir.
    return max(1.0, min(99.0, sum(candidates) / len(candidates) * 100.0))


def _market_odds_consensus(odds_parsed, market):
    """Yaygın API-Football market adlarını destekler."""
    aliases = {
        "MS1": (["Match Winner", "1X2"], ["home"]),
        "X": (["Match Winner", "1X2"], ["draw"]),
        "MS2": (["Match Winner", "1X2"], ["away"]),
        "OVER15": (["Goals Over/Under", "Over/Under"], ["Over 1.5"]),
        "UNDER15": (["Goals Over/Under", "Over/Under"], ["Under 1.5"]),
        "OVER25": (["Goals Over/Under", "Over/Under"], ["Over 2.5"]),
        "UNDER25": (["Goals Over/Under", "Over/Under"], ["Under 2.5"]),
        "OVER35": (["Goals Over/Under", "Over/Under"], ["Over 3.5"]),
        "UNDER35": (["Goals Over/Under", "Over/Under"], ["Under 3.5"]),
        "KG": (["Both Teams Score", "Both Teams To Score"], ["Yes"]),
        "KG_YOK": (["Both Teams Score", "Both Teams To Score"], ["No"]),
    }
    if market not in aliases:
        return None
    return _odds_probability(odds_parsed, *aliases[market])


def _blend_sources(items):
    """(olasılık, ağırlık) kaynaklarını güvenilir ağırlıklı ortalamaya çevirir."""
    valid = [(float(p), max(0.0, float(w))) for p, w in items if p is not None and w > 0]
    if not valid:
        return 50.0
    total_w = sum(w for _, w in valid)
    return sum(p * w for p, w in valid) / total_w


def _confidence_weight(sample_size, minimum=0.35, maximum=1.0):
    n = max(0.0, float(sample_size))
    return max(minimum, min(maximum, math.sqrt(n / (n + 8.0))))


def gelistirilmis_model(poisson_sonuc, home_form, away_form,
                        home_venue, away_venue, prediction, h2h,
                        odds_parsed=None):
    """Profesyonel çok-kaynaklı ensemble model.

    Kaynaklar: Poisson, son form, ev/deplasman formu, API-Football tahmini
    ve varsa bookmaker konsensüsü. Her kaynak örneklemine göre ağırlıklandırılır.
    Amaç yüksek görünen rakam üretmek değil, farklı kaynakların aynı yöndeki
    sinyalini daha belirgin hale getirmek ve çelişkilerde güveni azaltmaktır.
    """
    p = dict(poisson_sonuc)
    odds_parsed = odds_parsed or {}
    ...
    def pct_from_record(rec, key):
        n = int(num(rec.get("mac", 0), 0) or 0)
        return num(rec.get(key), 0) / n * 100 if n else None

    hf = max(0, int(num(home_form.get("mac", 0), 0) or 0))
    af = max(0, int(num(away_form.get("mac", 0), 0) or 0))
    hv = max(0, int(num(home_venue.get("mac", 0), 0) or 0))
    av = max(0, int(num(away_venue.get("mac", 0), 0) or 0))

    # ---------------------------
    # 1X2
    # ---------------------------
    ms1_items = [(p["MS1"], 1.00)]
    x_items = [(p["X"], 1.00)]
    ms2_items = [(p["MS2"], 1.00)]

    for target, items in (
        ("MS1", ms1_items), ("X", x_items), ("MS2", ms2_items)
    ):
        if target == "MS1":
            vals = [pct_from_record(home_venue, "G"), pct_from_record(home_form, "G"),
                    pct_from_record(away_venue, "M"), pct_from_record(away_form, "M")]
        elif target == "X":
            vals = [pct_from_record(home_venue, "B"), pct_from_record(home_form, "B"),
                    pct_from_record(away_venue, "B"), pct_from_record(away_form, "B")]
        else:
            vals = [pct_from_record(home_venue, "M"), pct_from_record(home_form, "M"),
                    pct_from_record(away_venue, "G"), pct_from_record(away_form, "G")]
        for v in vals:
            if v is not None:
                items.append((v, 0.55))
    ...
    if prediction:
        ...
        for key, items, api_key in (("MS1", ms1_items, "home"), ("X", x_items, "draw"), ("MS2", ms2_items, "away")):
            v = num(percent.get(api_key))
            if v is not None:
                items.append((v, 0.75))

    for key, items in (("MS1", ms1_items), ("X", x_items), ("MS2", ms2_items)):
        odd = _market_odds_consensus(odds_parsed, key)
        if odd is not None:
            items.append((odd, 0.90))

    ms1 = _blend_sources(ms1_items)
    msx = _blend_sources(x_items)
    ms2 = _blend_sources(ms2_items)
    total = ms1 + msx + ms2 or 1.0
    ms1, msx, ms2 = ms1 / total * 100, msx / total * 100, ms2 / total * 100

    # H2H yalnızca yeterli sayıda geçmiş maç varsa düşük ağırlıkla kullanılır.
    # Böylece eski kadroların etkisi bugünkü formun önüne geçmez.
    h2h_stats = {"n": 0, "home_win": None, "draw": None, "away_win": None, "over25": None, "kg": None}
    if h2h:
        vals = []
        for m in h2h[:10]:
            ...
            if gh is None or ga is None:
                continue
            try:
                vals.append((int(gh), int(ga)))
            except Exception:
                continue
        if len(vals) >= 4:
            n = len(vals)
            h2h_stats["n"] = n
            h2h_stats["home_win"] = sum(a > b for a, b in vals) / n * 100
            h2h_stats["draw"] = sum(a == b for a, b in vals) / n * 100
            h2h_stats["away_win"] = sum(a < b for a, b in vals) / n * 100
            h2h_stats["over25"] = sum(a + b >= 3 for a, b in vals) / n * 100
            h2h_stats["kg"] = sum(a > 0 and b > 0 for a, b in vals) / n * 100
            # H2H'nin taraf sonucu yönü, geçmiş maçlardaki ev/deplasman
            # dizilimi değişebildiği için 1X2 modeline doğrudan eklenmez.
            # Sadece toplam gol ve KG gibi yönsüz pazarlar için kullanılır.

    # ---------------------------
    # Gol pazarları
    # ---------------------------
    def empirical(key_w, key_plain):
        vals = [home_form.get(key_w), away_form.get(key_w), home_form.get(key_plain), away_form.get(key_plain)]
        vals = [num(x) for x in vals if num(x) is not None]
        return sum(vals) / len(vals) if vals else None

    def goal_market(key, empirical_key, poisson_key):
        items = [(p[poisson_key], 1.25)]
        emp = empirical(*empirical_key) if empirical_key else None
        if emp is not None:
            # Güncel formun etkisi örneklem büyüklüğü ile artar, ama Poisson omurga kalır.
            n = min(hf, af)
            items.append((emp, 0.55 + 0.35 * _confidence_weight(n)))
        odd = _market_odds_consensus(odds_parsed, key)
        if odd is not None:
            items.append((odd, 0.95))
        return _blend_sources(items)

    over15 = goal_market("OVER15", ("W_OVER15", "OVER15"), "OVER15")
    over25 = goal_market("OVER25", ("W_OVER25", "OVER25"), "OVER25")
    kg = goal_market("KG", ("W_KG", "KG"), "KG")
    if h2h_stats["n"]:
        over25 = _blend_sources([(over25, 1.0), (h2h_stats["over25"], 0.16)])
        kg = _blend_sources([(kg, 1.0), (h2h_stats["kg"], 0.16)])
    over35 = _blend_sources([(p["OVER35"], 1.35),
                             (_market_odds_consensus(odds_parsed, "OVER35"), 0.90)])

    # 0.5 üstü Poisson'dan gelir; odds varsa hafif doğrulama.
    over05 = p["OVER05"]

    p["MS1"], p["X"], p["MS2"] = ms1, msx, ms2
    p["OVER05"] = max(0.0, min(100.0, over05))
    p["UNDER05"] = 100 - p["OVER05"]
    p["OVER15"] = max(0.0, min(100.0, over15))
    p["UNDER15"] = 100 - p["OVER15"]
    p["OVER25"] = max(0.0, min(100.0, over25))
    p["UNDER25"] = 100 - p["OVER25"]
    p["OVER35"] = max(0.0, min(100.0, over35))
    p["UNDER35"] = 100 - p["OVER35"]
    p["KG"] = max(0.0, min(100.0, kg))
    p["KG_YOK"] = 100 - p["KG"]

    # İlk gol + golsüz senaryo aynı olay ağacından normalize edilir.
    first_home = max(0.0, min(100.0, p["FIRST_HOME"]))
    first_away = max(0.0, min(100.0, p["FIRST_AWAY"]))
    no_goal = max(0.0, min(100.0, p["NO_GOAL"]))
    first_sum = first_home + first_away + no_goal
    if first_sum > 0:
        p["FIRST_HOME"] = first_home / first_sum * 100
        p["FIRST_AWAY"] = first_away / first_sum * 100
        p["NO_GOAL"] = no_goal / first_sum * 100

    # Daha yüksek çizgilerin olasılığı mantıksal olarak düşmelidir.
    # 1.5 >= 2.5 >= 3.5 ve altlar tamamlayıcıdır.
    p["OVER15"] = max(p["OVER25"], p["OVER15"])
    p["OVER25"] = max(p["OVER35"], min(p["OVER25"], p["OVER15"]))
    p["UNDER15"] = 100 - p["OVER15"]
    p["UNDER25"] = 100 - p["OVER25"]
    p["UNDER35"] = 100 - p["OVER35"]

    # Kaynak desteği: UI güven seviyesinde kullanılır.
    def support(items, final):
        vals = [v for v, _ in items if v is not None]
        if not vals:
            return (0, 0)
        if final >= 50:
            return (sum(v >= 50 for v in vals), len(vals))
        return (sum(v < 50 for v in vals), len(vals))

    p["DESTEK"] = {
        "MS1": support(ms1_items, p["MS1"]),
        "X": support(x_items, p["X"]),
        "MS2": support(ms2_items, p["MS2"]),
        "OVER15": (0, 0), "OVER25": (0, 0), "KG": (0, 0), "OVER35": (1, 1),
    }
    return p
...
def market_onerileri_olustur(poisson_sonuc, home_corner, away_corner,
                             ...
                             odds_parsed=None):
    """Yüksek olasılıklı pazarları çok-kaynaklı model ile sıralar."""
    ...
    def add(markets, label, key, source):
        value = float(model.get(key, 0) or 0)
        markets.append((label, max(0.0, min(100.0, value)), source, key))

    markets = []
    add(markets, "MS1", "MS1", "Poisson + form + saha + API + piyasa")
    add(markets, "MS X", "X", "Poisson + form + saha + API + piyasa")
    add(markets, "MS2", "MS2", "Poisson + form + saha + API + piyasa")
    add(markets, "0.5 ÜSTÜ", "OVER05", "Poisson gol modeli")
    add(markets, "0.5 ALTI", "UNDER05", "Poisson gol modeli")
    add(markets, "1.5 ÜSTÜ", "OVER15", "Poisson + güncel form + piyasa")
    add(markets, "1.5 ALTI", "UNDER15", "Poisson + güncel form + piyasa")
    add(markets, "2.5 ÜSTÜ", "OVER25", "Poisson + form + piyasa")
    add(markets, "2.5 ALTI", "UNDER25", "Poisson + form + piyasa")
    add(markets, "3.5 ÜSTÜ", "OVER35", "Poisson + piyasa")
    add(markets, "3.5 ALTI", "UNDER35", "Poisson + piyasa")
    add(markets, "KG VAR", "KG", "Poisson + güncel form + piyasa")
    add(markets, "KG YOK", "KG_YOK", "Poisson + güncel form + piyasa")
    add(markets, f"İLK GOL - {home_name}", "FIRST_HOME", "İlk gol Poisson modeli")
    add(markets, f"İLK GOL - {away_name}", "FIRST_AWAY", "İlk gol Poisson modeli")

    # Çifte şans, 1X/X2/12 gibi tamamlayıcı pazarlar yüksek olasılıklı
    # alternatifleri görünür kılar.
    markets.extend([
        ("ÇİFTE ŞANS 1X", model["MS1"] + model["X"], "1X2 birleşik model", "DOUBLE_1X"),
        ("ÇİFTE ŞANS X2", model["X"] + model["MS2"], "1X2 birleşik model", "DOUBLE_X2"),
        ("ÇİFTE ŞANS 12", model["MS1"] + model["MS2"], "1X2 birleşik model", "DOUBLE_12"),
    ])

    corner_model = gelismis_korner_model(home_corner, away_corner)
    if corner_model:
        for key, label in (
            ("over75", "7.5 KORNER ÜSTÜ"), ("over85", "8.5 KORNER ÜSTÜ"),
            ("over95", "9.5 KORNER ÜSTÜ"), ("over105", "10.5 KORNER ÜSTÜ"),
            ("over115", "11.5 KORNER ÜSTÜ"), ("over125", "12.5 KORNER ÜSTÜ"),
        ):
            pct = float(corner_model["lines"][key])
            markets.append((label, pct, f"Gelişmiş korner dağılımı | {corner_model['mac']} maç", "CORNER"))
            markets.append((label.replace("ÜSTÜ", "ALTI"), 100 - pct,
                            f"Gelişmiş korner dağılımı | {corner_model['mac']} maç", "CORNER"))

    # En yüksek olasılığı değil, yüksek olasılık + kaynak uyumunu öne al.
    # Çok yakın yüzdelerde küçük farklar yaratmak için yapay bonus verilmez.
    markets.sort(key=lambda x: (float(x[1]), x[0]), reverse=True)
    return markets
...
def mac_analizi(mac):
    ...
    analiz_penceresi.title(f"📊 {home_name} - {away_name}")
    analiz_penceresi.geometry("1220x900")
    analiz_penceresi.minsize(1000, 700)
    ...
    tk.Label(hero, text="MAÇ ANALİZ RAPORU", bg=PANEL, fg=ACCENT,
             font=("Segoe UI", 9, "bold")).pack(pady=(12, 2))
    tk.Label(hero, text=f"{home_name}  ︱  {away_name}", bg=PANEL, fg=TEXT,
             font=("Segoe UI", 23, "bold")).pack(pady=(0, 3))
    tk.Label(
        hero,
    ...
    progress_label = tk.Label(
        status_bar, text="⏳  Analiz hazırlanıyor...",
    ...
    text_frame.pack(fill="both", expand=True, padx=16, pady=(0, 14))
    ...
    analiz_text = tk.Text(
        ...
        spacing1=2, spacing3=2, yscrollcommand=scrollbar.set
    )
    ...
    analiz_text.tag_add("hero", "1.0", "1.end")
    ...
    def yaz(metin):
        ...
        for raw in metin.splitlines():
            ...
            if any(k in stripped for k in (
                ...
                "POISSON MODELİ", "API-FOOTBALL TAHMİNİ",
                "MAÇ ORANLARI", "MODEL ÖZETİ",
                "MODELİN EN YÜKSEK OLASILIKLI PAZARLARI"
            )):
                analiz_text.insert("end", "\n" + stripped + "\n", "section")
                continue
            if stripped.startswith((
                "Tahmini skor:", "Tahmin:", "Model skor:",
    ...
    def worker():
        ...
        try:
            ...
            # Hücum gücü ile rakip savunmasını ayrı hesapla. Geometrik ortalama,
            # tek bir uç değerin lambda'yı aşırı şişirmesini engeller.
            home_attack = home_recent_gf * 0.55 + home_venue_gf * 0.45
            home_defence = home_recent_ga * 0.45 + home_venue_ga * 0.55
            away_attack = away_recent_gf * 0.55 + away_venue_gf * 0.45
            away_defence = away_recent_ga * 0.45 + away_venue_ga * 0.55

            home_lambda = math.sqrt(max(0.05, home_attack) * max(0.05, away_defence))
            away_lambda = math.sqrt(max(0.05, away_attack) * max(0.05, home_defence))

            # Küçük örneklemde aşırı uçları lige/genel futbola yakın nötr bir
            # önsel ile hafifçe yumuşatıyoruz. Bu bir garanti değil, varyans kontrolüdür.
            home_n = min(1.0, (hf_games + hv) / 24.0)
            away_n = min(1.0, (af_games + av) / 24.0)
            home_lambda = home_lambda * (0.15 + 0.85 * home_n) + 1.35 * (0.85 - 0.85 * home_n)
            away_lambda = away_lambda * (0.15 + 0.85 * away_n) + 1.05 * (0.85 - 0.85 * away_n)
