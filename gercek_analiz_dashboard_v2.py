...
def mac_analizi(mac):
    ...
    analiz_penceresi.geometry("1280x900")
    ...
    tk.Label(section_bar, text="Model  •  Form  •  Korner  •  H2H  •  API  •  Oran",
             bg=PANEL, fg=MUTED, font=("Segoe UI", 8)).pack(side="right", padx=12)

    detail_outer = tk.Frame(analiz_penceresi, bg=BG)
    detail_outer.pack(fill="both", expand=True, padx=18, pady=(0, 16))

    detail_canvas = tk.Canvas(detail_outer, bg=BG, highlightthickness=0, bd=0)
    ...
    detail_grid = tk.Frame(detail_canvas, bg=BG)
    detail_window = detail_canvas.create_window((0, 0), window=detail_grid, anchor="nw")
    detail_canvas.configure(yscrollcommand=detail_scroll.set)

    def detail_resize(event):
        detail_canvas.itemconfig(detail_window, width=event.width)
    detail_canvas.bind("<Configure>", detail_resize)
    detail_grid.bind("<Configure>", lambda e: detail_canvas.configure(scrollregion=detail_canvas.bbox("all")))

    detail_loading = tk.Frame(detail_grid, bg=CARD,
                              highlightbackground=BORDER, highlightthickness=1)
    detail_loading.grid(row=0, column=0, columnspan=2, sticky="ew", padx=4, pady=4)
    tk.Label(detail_loading, text="⏳ ANALİZ MOTORU ÇALIŞIYOR",
             bg=CARD, fg=ACCENT, font=("Segoe UI", 12, "bold")).pack(anchor="w", padx=18, pady=(18, 4))
    tk.Label(detail_loading,
             text="Son maçlar • form • korner • H2H • puan durumu • API tahmini • oranlar • Poisson modeli",
             bg=CARD, fg=MUTED, font=("Segoe UI", 9)).pack(anchor="w", padx=18, pady=(0, 18))

    def make_detail_card(parent, title, lines, icon="📌", column=0):
        box = tk.Frame(parent, bg=CARD,
                       highlightbackground=BORDER, highlightthickness=1)
        box.grid(row=0, column=column, sticky="nsew", padx=5, pady=5)
        head = tk.Frame(box, bg=PANEL_2)
        head.pack(fill="x")
        tk.Label(head, text=f"{icon}  {title}", bg=PANEL_2, fg=TEXT,
                 font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=14, pady=9)
        ...
        for line in lines:
            ...
            fg = "#dbeafe"
            font = ("Segoe UI", 9)
            if any(x in line for x in ("ÇOK GÜÇLÜ", "GÜÇLÜ", "YÜKSEK")):
                fg, font = ACCENT_2, ("Segoe UI", 9, "bold")
            elif any(x in line for x in ("ORTA", "SINIRDA")):
                fg, font = WARNING, ("Segoe UI", 9, "bold")
            elif line.startswith(("Tahmini skor", "Model skor", "Beklenen gol", "Model beklenen gol")):
                fg, font = TEXT, ("Segoe UI", 10, "bold")
            elif line.startswith("Not:"):
                fg, font = MUTED, ("Segoe UI", 8)
            tk.Label(body, text=line, bg=CARD, fg=fg, font=font,
                     anchor="w", justify="left", wraplength=560).pack(fill="x", pady=2)
        return box

    def parse_sections(metin):
        headers = {
            "🏠": ("EV SAHİBİ", "🏠"),
            "✈️": ("DEPLASMAN", "✈️"),
            "🚩 KORNER ANALİZİ": ("KORNER ANALİZİ", "🚩"),
            "🔮 KORNER MODELİ": ("KORNER MODELİ", "🔮"),
            "📊 POISSON MODELİ": ("POISSON MODELİ", "📊"),
            "🤝 SON H2H MAÇLARI": ("SON H2H MAÇLARI", "🤝"),
            "🏆 PUAN DURUMU": ("PUAN DURUMU", "🏆"),
            "🤖 API-FOOTBALL TAHMİNİ": ("API-FOOTBALL TAHMİNİ", "🤖"),
            "💰 MAÇ ORANLARI": ("MAÇ ORANLARI", "💰"),
            "📌 MODEL ÖZETİ": ("MODEL ÖZETİ", "📌"),
        }
        sections = []
        current = None
        for raw in metin.splitlines():
            ...
            if not line or line.startswith(("═", "─", "━")):
                continue
            if line.startswith("🎯 ÖNE ÇIKAN MODEL SONUÇLARI"):
                continue
            matched = None
            ...
            if matched:
                ...
                current = {"title": matched[0], "icon": matched[1], "lines": []}
                if matched[0] == "EV SAHİBİ":
                    current["title"] = line.replace("🏠", "").strip()
                elif matched[0] == "DEPLASMAN":
                    current["title"] = line.replace("✈️", "").strip()
                continue
    ...
    def yaz(metin):
        ...
        for widget in detail_grid.winfo_children():
            widget.destroy()
        detail_grid.columnconfigure(0, weight=1)
        detail_grid.columnconfigure(1, weight=1)

        sections = parse_sections(metin)
        if not sections:
            make_detail_card(detail_grid, "ANALİZ", [metin], "📊", 0).grid_configure(columnspan=2)
        else:
            for i, section in enumerate(sections):
                row, col = divmod(i, 2)
                box = make_detail_card(detail_grid, section["title"], section["lines"], section["icon"], col)
                box.grid_configure(row=row, column=col)

        footer = tk.Frame(detail_grid, bg=PANEL_2,
                          highlightbackground=BORDER, highlightthickness=1)
        footer.grid(row=((len(sections)+1)//2), column=0, columnspan=2,
                    sticky="ew", padx=5, pady=(10, 5))
        tk.Label(footer,
                 text="ℹ️ Model sonuçları istatistiksel verilerden üretilir; kesinlik veya garanti anlamına gelmez.",
                 bg=PANEL_2, fg=MUTED, font=("Segoe UI", 8)).pack(anchor="w", padx=14, pady=10)
        detail_canvas.update_idletasks()
        detail_canvas.yview_moveto(0)
        progress_label.config(text="✓  Analiz tamamlandı")

    # Fare tekerleği yalnızca analiz panelinde çalışsın.
    def detail_wheel(event):
        detail_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")
    detail_canvas.bind("<MouseWheel>", detail_wheel)
