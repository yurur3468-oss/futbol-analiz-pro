...
try:
    if "API_FOOTBALL_KEY" in st.secrets:
        secret_key = str(st.secrets["API_FOOTBALL_KEY"]).strip()

        if secret_key:
            os.environ["API_FOOTBALL_KEY"] = secret_key
...
st.markdown(
    """
    <div class="brand-sub">
        Profesyonel Futbol Analiz Merkezi
    </div>
    """,
    unsafe_allow_html=True
)
...
if not API_KEY:
    ...
    st.error(
        ...
        "Streamlit Secrets bölümünde API_FOOTBALL_KEY tanımlı olmalı."
    )
...
if not matches:
    ...
    st.info(
        "Tarih seçip **MAÇLARI GETİR** butonuna bas."
    )
